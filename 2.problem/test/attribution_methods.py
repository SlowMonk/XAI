...
class CAMS(object):
    ...
    def generate_image(self, img, i):
        ...
        with self.returnCAM(self.feature_blobs[0],self.weight_softmax,[idx[0].item()]) as output_cam:
       ...
       # print('camresult->{}'.format(camsresult.shape))
    ...
    # TODO problem1 idx problem
            with self.generate_image(img, 0) as result:
...
class CAMS_MNISTb(object):
    ...
    def generate_image(self, img, i):
        ...
        #return camsresult,heatmap,probs.detach().cpu().numpy(), idx.detach().cpu().numpy()
        result
    # TODO : check the memory problem stacking the saliency map.
    #@contextmanager
    def save_saliency_map(self,dataloader,save_dir):
        ...
        for idx, (img, target) in enumerate(dataloader):
            if idx%500==0:
                print(idx/50000,'%')
            sal_maps_b, heat_map_b, probs_b, preds_b = self.generate_image(img, 0)
            ...
            del sal_maps_b,heat_map_b,probs_b,preds_b
            gc.collect()
...
class CAMS_MNIST3(object):
    ...
    def save_saliency_map(self,dataloader,save_dir):
        ...
        for idx, (img, target) in enumerate(dataloader):
            if idx%500==0:
                hf = h5py.File(f'./datab/file_mnist.hdf5', 'r')
